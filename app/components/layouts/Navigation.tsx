          ...
          {lists.map((list) => (
            list.showFlag && (
              <li key={list.href}>
                <Link 
                  href={list.href}
                  className={`${list.pathname === pathname && "bg-blue-500 font-semibold"} flex items-center justify-center rounded-full w-[40px] h-[40px] px-2 py-2 transition duration-200 ease-in-out cursor-pointer hover:bg-blue-500`}
                >
                  <Icon icon={list.icon} color="#fff" size={20} />
                </Link>
              </li>
