          ...
          {lists.map((list) => (
            list.showFlag && (
              <li key={list.href}>
                <Link 
                  href={list.href}
                  className={`${list.pathname === pathname && "bg-blue-500 font-semibold"} flex items-center gap-4 rounded-sm px-4 py-2 transition duration-200 ease-in-out cursor-pointer hover:where:bg-blue-500 active:bg-blue-600`}
                >
